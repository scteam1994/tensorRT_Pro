...
namespace AlphaPose{
    ...
    struct AffineMatrix{
        ...
        void compute(const cv::Size& image_size, const cv::Rect& box, const cv::Size& net_size){
            ...
            i2d[0] = scale;  i2d[1] = 0;      i2d[2] = -((box_.x + box_.width + box_.x - pad_width)/2 - box_.width  * 1 * rate + pad_width * 0.5)  * scale + net_size.width  * 0.5 + scale * 0.5 - 0.5;  
            i2d[3] = 0;      i2d[4] = scale;  i2d[5] = -((box_.y + box_.height + box_.y - pad_height)/2 - box_.height * 1 * rate + pad_height * 0.5) * scale + net_size.height * 0.5 + scale * 0.5 - 0.5;
